...
def main(args, config):
    ...
    mask_modal = config['mask_modal'] if 'mask_modal' in config else ""
    train_datasets = MedKLIP_Dataset(config['train_file'],config['label_file'],config['dis_label_file'],config['report_observe'], mode = 'train', augmentation=augment,mask_modal=mask_modal, modfilter=[1,0,1,0])
    ...
    val_datasets = MedKLIP_Dataset(config['valid_file'],config['label_file'],config['dis_label_file'],config['report_observe'],mode ='valid',mask_modal=mask_modal,modfilter=[1,0,1,0])
    ...
    if len(args.finetune_checkpoint):    
        ...
        for idx in range(4):
            image_encoder[idx].load_state_dict(checkpoint['image_encoder'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='/remote-home/mengxichen/UniBrain-lora/Pretrain/output_missft/output_mod13v1_full')
