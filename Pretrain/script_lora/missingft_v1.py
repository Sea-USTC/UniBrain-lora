...
from models.lora4missing.imageEncoder import ModelRes, ModelDense
...
def main(args, config):
    ...
    mask_modal = config['mask_modal'] if 'mask_modal' in config else ""
    train_datasets = MedKLIP_Dataset(config['train_file'],config['label_file'],config['dis_label_file'],config['report_observe'], mode = 'train', augmentation=augment,mask_modal=mask_modal, modfilter=[0,1,0,0])
    ...
    val_datasets = MedKLIP_Dataset(config['valid_file'],config['label_file'],config['dis_label_file'],config['report_observe'],mode ='valid',mask_modal=mask_modal,modfilter=[0,1,0,0])
    ...
    clip_loss = ClipLoss(mask_modal=mask_modal)
    print(image_encoder.state_dict().keys())
    print("#######################################")
    ...
    optimizer = create_optimizer(arg_opt, model, image_encoder, text_encoder, fuseModule, clip_loss=None)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--finetune_checkpoint', default='/remote-home/mengxichen/UniBrain-lora/Pretrain/output_missing/output_baselinev1/best_val.pth')
    parser.add_argument('--output_dir', default='/remote-home/mengxichen/UniBrain-lora/Pretrain/output_misslora/output_mod2v1')
