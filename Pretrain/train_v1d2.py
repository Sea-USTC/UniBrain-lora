...
def train(model, image_encoder, text_encoder, fuseModule, tokenizer, data_loader, optimizer, epoch, warmup_steps, device, scheduler, config, clip_loss):
    ...
    for i, sample in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss.backward()
        optimizer.step()
        if clip_loss.logit_scale.data> np.log(100):
            clip_loss.logit_scale.data = np.log(100)
...
def main(args, config):
    ...
    optimizer = create_optimizer(arg_opt, model, image_encoder, text_encoder, None, fuseModule, clip_loss=clip_loss)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/GPFS/rhome/hanchongyan/UniBrain-lora/Pretrain/configs/config_fifteen.yaml')
    parser.add_argument('--finetune_checkpoint', default='')
    parser.add_argument('--output_dir', default='/GPFS/rhome/hanchongyan/UniBrain-lora/Pretrain/output_fifteen/output_baselinev3')
