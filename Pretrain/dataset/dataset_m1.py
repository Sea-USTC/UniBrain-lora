...
class MedKLIP_Dataset(Dataset):
    def __init__(self, data_path, label_path, dis_label_path, report_observe, mode = 'train', augmentation=False, only_global=False,mask_modal="", modfilter=None):
        ...
        for k in self.ann:
            for modal in ["DWI","T1WI","T2WI","T2FLAIR"]:
                self.ann[k][modal]="/remote-home/share/data117"+self.ann[k][modal][34:]
        ...
        if os.path.exists(miss_path):
            ...
            self.randmiss = np.random.randint(1,16,self.totlen)
            np.save(miss_path,self.randmiss)
    ...
    def __getitem__(self, index):
        ...
        if self.augmentation:
            images = []
            for idx, modal in enumerate(modal_dic):
                if modal_state[idx]==0 or self.ann[fid][modal] == '':
