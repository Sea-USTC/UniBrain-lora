...
from models.lora4missing.resnet import resnet,densenet
from models.lora4missing.lora import LoRALinear
...
class ModelRes(nn.Module):
    def __init__(self, config):
        ...
        self.resnet = self._get_res_base_model(config['r_ratio'],config['lora_alpha'],config['model_type'],config['model_depth'],config['input_W'],
        ...
        self.res_l1 = LoRALinear(num_ftrs*config['r_ratio'],config['lora_alpha'],num_ftrs, num_ftrs)
        self.res_l2 = LoRALinear(num_ftrs*config['r_ratio'],config['lora_alpha'],num_ftrs, out_feature)

    def _get_res_base_model(self, r_ratio, lora_alpha, model_type,model_depth,input_W,input_H,input_D,resnet_shortcut,no_cuda,gpu_id,pretrain_path,out_feature):
        if model_depth == 10:
